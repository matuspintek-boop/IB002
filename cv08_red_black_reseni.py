...
def search_rec(node: Node | None, key: Any) -> Node | None:
...
def search(tree: RedBlackTree, key: Any) -> Node | None:
...
def black_height(node: Node | None) -> int:
    if node is None:
        return 0
...
def is_correct_rb_tree_rec(
    node: Node | None, min_val: float, max_val: float, height: int
) -> bool:
    if node is None:
        return height == 0
...
def is_correct_rb_tree_alt_rec(
    node: Node | None, min_val: float, max_val: float
) -> tuple[bool, int]:
...
def make_graphviz(node: Node | None, dot_file: TextIO) -> None:
    if node is None:
        return
