...
def insert(linked_list: LinkedList, value: Any) -> Node:
    ...
    if linked_list.first is None:
        ...
        return node  # kosmeticka uprava: early return predstavuje citelnejsi kod

    tmp = linked_list.first
    # CHYBA: nalezeni posledniho prvku musim udelat tak,
    # abych jej nezapomnel
...
def delete_key(linked_list: LinkedList, key: Any) -> bool:
    ...
    previous = None
    # kosmeticka oprava: pro lepsi citelnost se daji obe podminky
    # zapsat do jedne
...
def multiply_numbers(bound: int, numbers: list[int]) -> int:
    ...
    for i in range(len(numbers)):
        # CHYBA: kontrola ze patri hodnota do intervalu
        if 1 <= numbers[i] <= bound:
            # CHYBA: posun pole o jedna doleva
            array[numbers[i] - 1] += 1
    val = 1
    for i in range(len(array)):
        # CHYBA: Interval od 1 do bound
        # oprava efektivity: nahrazeni nasobeni v cyklu umocnovanim
        val *= (i + 1) ** array[i]
    return val
...
def has_correct_parentheses(string: str) -> bool:
    ...
    for i in range(len(string)):
        ...
        if string[i] == ')':
            opened -= 1
        # CHYBA: pokud pocet otevrenych klesne pod 0, return False
        if opened < 0:
            return False
    # CHYBA: az zde se ma testovat, zdali je 0 otevrenych
    return opened == 0
...
def sequence_sum(sequence: list[int]) -> int:
    ...
    """
    strange_sum = 0
    # CHYBA: spatne rozsah, pro 0 pristup mimo meze
...
def find_substring(string: str, substring: str) -> int:
    ...
    # CHYBA: podretezec musime testovat od zacatku
    j = 0
    i = 0
    while i < len(string):
        if string[i] == substring[j]:
            if j == len(substring) - 1:
                return i - j
            j += 1
        # CHYBA: pokud se retezce na nejakem znaku zacnou neshodovat
