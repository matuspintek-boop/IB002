...
class Graph:
    ...
    def __init__(self, size: int) -> None:
        ...
        self.predecessors: list[int | None] = []
...
def bellman_ford(graph: Graph, u: int, v: int) -> float | None:
    ...
    for x in range(graph.size):
        for y in range(graph.size):
            if is_edge(graph, x, y) and relax(graph, x, y):
                # doslo ke zmene pri relaxaci: graf obsahuje zaporny cyklus
                return None
...
def dijkstra(graph: Graph, u: int, v: int) -> float:
    ...
    # oznaceni jiz zpracovanych vrcholu
    done = [False for _ in range(graph.size)]
    heap: list[tuple[float, int]] = [(0, u)]   # fronta dvojic vzdalenost, vrchol

    while heap:
        # prvni polozku z dvojice smime ignorovat, je to totiz zarucene
        # graph.distances[x]; ukladali jsme ji do prioritni fronty kvuli
        # spravnemu poradi
        _, x = heappop(heap)

        if done[x]:
            continue  # vrchol uz jsme drive zpracovali

        done[x] = True

        for y in range(graph.size):
            if not done[y] and is_edge(graph, x, y) and relax(graph, x, y):
                # zde bychom chteli volat decrease_key, pokud y uz v halde je
                # misto toho y vzdy znovu vlozime do haldy (s novou hodnotou)
                heappush(heap, (graph.distances[y], y))
...
def test_dijkstra() -> None:
    ...
    # Tests updating of priority queue
    graph5 = create_test_graph()
