...
#
# Krome testu je pro vas pripraven take vypis v jazyce dot. Staci z vaseho kodu
# zavolat funkci make_graph(graph, filename), ktere predate graf a jmeno
# souboru, do ktereho chcete graf vypsat.
#
# Datova struktura deque z knihovny collections je obousmerna fronta
# implementovana pomoci obousmerne zretezeneho seznamu. Pro ucely pouziti
# jako jednoduche fronty staci metody append (prida prvek na konec fronty)
# a popleft (odebere prvek ze zacatku fronty), jejichz casova slozitost
# je konstantni.
...
class Graph:
    ...
    def clear_flags(self) -> None:
        self.parent: list[int | None] = [None] * self.size
        self.distance: list[int | None] = [None] * self.size
        self.discovery_time: list[int | None] = [None] * self.size
        self.finishing_time: list[int | None] = [None] * self.size
...
def check_dfs(
    ...
    start: int,
    parent: list[int | None],
    discovery_time: list[int | None],
    finishing_time: list[int | None]
) -> bool:
    dfs(graph, start)
