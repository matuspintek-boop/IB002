...
def transplant(tree: BinarySearchTree, u: Node, v: Node | None) -> None:
    if u.parent is None:
        tree.root = v
...
def height_rec(node: Node | None) -> int:
    if node is None:
        return -1
...
def is_correct_bst_rec(
    node: Node | None, min_val: Any, max_val: Any
) -> bool:
    if node is None:
        return True
